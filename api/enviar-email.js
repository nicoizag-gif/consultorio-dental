...
export default async function handler(req, res) {
  ...
  const { to, paciente, fecha, hora, motivo, duracion, direccion, profesional, esRecordatorio } = req.body
  ...
  const ics = [
    ...
    `DTSTART;TZID=America/Argentina/Buenos_Aires:${fmtICS(fechaObj)}`,
    `DTEND;TZID=America/Argentina/Buenos_Aires:${fmtICS(fechaFin)}`,
    `SUMMARY:Turno odontologico - ${profesional || 'Consultorio'}`,
    `DESCRIPTION:Prestacion: ${motivo}\\nDuracion: ${duracion || 60} min`,
    `LOCATION:${direccion || ''}`,
    'END:VEVENT',
    'END:VCALENDAR'
  ].join('\r\n')
      ...
      <div style="background: #f8f8f6; padding: 24px; border-radius: 0 0 10px 10px; border: 1px solid #eee; border-top: none;">
        ...
        <div style="background: #fff; border-radius: 8px; padding: 16px; border: 1px solid #eee; margin-bottom: 16px;">
          <table style="width: 100%; font-size: 14px; color: #333;">
            ...
            <tr><td style="padding: 6px 0; color: #888;">Prestacion</td><td style="padding: 6px 0;">${motivo}</td></tr>
            <tr><td style="padding: 6px 0; color: #888;">Duracion</td><td style="padding: 6px 0;">${duracion || 60} minutos</td></tr>
            ${direccion ? `<tr><td style="padding: 6px 0; color: #888;">Direccion</td><td style="padding: 6px 0;">${direccion}</td></tr>` : ''}
          </table>
        </div>
        ...
        <p style="font-size: 13px; color: #555; margin: 0;">
          ...
          <strong>${profesional || 'Consultorio odontologico'}</strong>
        </p>
      </div>
    </div>
  `
